...
6. **AIとの協業における規律 (Discipline in AI Collaboration)**: 本プロジェクトは、AIアシスタントとの協業を、開発プロセスの標準的な一部として位置づける。その協業は、`docs/99_ai_collaboration_guideline.md` で定義されたプロセスに厳格に従う。人間が戦略的判断の主導権を握り、AIを有効なツールとして活用することで、開発の品質と速度を両立させる。
...
5. **テストデータの一貫性と品質**: テストに使用するデータは、関連する仕様書（例: `12_wbs_task_syntax_specification.md`）に厳密に準拠すること。意図的に仕様から外れたデータ（エラーケース）をテストする場合は、その逸脱が意図的であることをテストコード内で明確に文書化する。
...
| 日付       | バージョン | 担当者    | 変更内容   |
|------------|------------|-----------|------------|
| 2025-06-10 | 1.3        | IT Legacy | AI協業をプロジェクトの標準プロセスとして位置づけるため、「開発哲学」にAI協業規律を明記し、旧「AIアシスタントとの協業ガイドライン（オプション）」セクションを削除。思想的一貫性を強化。 |
