...
* **理由**: PowerShell 7.5.1とPester 5.7.1の組み合わせにおいて、ファイルベースのテスト実行時に `Should` 構文で `ParseException` が発生する未解決の問題に直面したため。インタラクティブ実行やスクリプトブロック直接実行では問題なかったが、ファイルからの実行で問題が再現した。
